...
def four_point_transform(image, poly, visual=False):
    """
...
class ReferenceImage:
    ...
    def calculate_phash(self):
        ...
        self.phash = imagehash.phash(
            ...
            hash_size=32)
        #print('Calculated hash:',self.phash)
...
class TestImage:
    ...
    def plot_image_with_recognized(self, output_path, visual=False):
        ...
        plt.savefig(output_path + '/MTG_card_recognition_results_' +
                    str(self.name.split(FILE_EXT)[0]) +
                    FILE_EXT, dpi=600)
        if visual:
            plt.show()
...
class MagicCardDetector:
    ...
    def read_and_adjust_reference_images(self, path):
        ...
        print('...', end=' ')
        filenames = glob.glob(path + '*' + FILE_EXT)
    ...
    def read_and_adjust_test_images(self, path):
        ...
        print('Reading images from ' + str(path))
        print('...', end=' ')
        filenames = glob.glob(path.rstrip('/') + '/*' + FILE_EXT)
        print(len(filenames), 'files found.')
        for filename in filenames:
            img = cv2.imread(filename)
    ...
    def segment_image(self, test_image, contouring_mode='gray', visual=False):
        """
    ...
    def run_recognition(self, image_index=None):
        ...
        for i in image_index:
            ...
            for alg in alg_list:
                self.recognize_cards_in_image(test_image, alg, self.visual)
    ...
    def recognize_cards_in_image(self, test_image, contouring_mode, visual=False):
        """
        ...
        self.segment_image(test_image,visual=visual, contouring_mode=contouring_mode)
